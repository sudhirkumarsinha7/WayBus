...
let seatType = [
  {name: 'AC', icon: Images.ac, id: 1},
  {name: 'Non-AC', icon: Images.nonAc, id: 2},
  {name: 'Sleeper', icon: Images.sleeper, id: 3},
  {name: 'Seater', icon: Images.seater, id: 4},
];
class Home extends Component {
  ...
  sendRequest(val) {
    this.setState({
      modalTitle: val.name,
      modalSelectedData: val,
      modalSelectedIcon: val.icon,
    });
  ...
  setCity(val) {
    Keyboard.dismiss()
    let {selectedCitySearch, originId, destId} = this.state;
    if (this.state.selectedCitySearch === 1) {
      if (val[0] === originId) {
        Toast.show('Source and Destination can not be same!');
        return false;
      }
    }
  ...
  renderItem(item, i) {
    return (
      <TouchableOpacity
        ...
        <Text
          style={
            this.state.modalSelectedData
              ? item.id === this.state.modalSelectedData.id
                ? styles.selectedTextBlue
                : styles.selectedText
              : styles.selectedText
          }>
          {item.name}
  ...
  render() {
    ...
    const cityData = this.state.searchText ? searchedCities : allCities;
    return (
      <View style={styles.mainContainer}>
        ...
        <FilterModal1
          ...
          closeModal={() => this.setState({filterModalVisible: !filterModalVisible})}
          idKey={'name'}
        />
          ...
          <View
            ...
            <View
              ...
              <TouchableOpacity
                ...
                onPress={() => this.setState({showCities: false})}>
                <Image
              ...
              <TextInput
                ...
                onChangeText={(val) => {
                  ...
                  allCities.forEach((item) => {
                    var len = val.length;
                    if (
                      item[1].toLowerCase().slice(0, len) == val.toLowerCase()
                    ) {
              ...
              />
              {searchedCities.length>0?<View>
              <Text style={{fontWeight: 'bold', forntSize: 16, padding: 20}}> Recent Search</Text>
              <FlatList
                ...
                data={searchedCities}
                renderItem={({item, index}) => this.renderCitiesData(item, index)}
              />
              </View>: null}
              <Text style={{fontWeight: 'bold', forntSize: 16, padding: 20}} > Top Cities</Text>
              <FlatList
                showsVerticalScrollIndicator={false}
                keyboardShouldPersistTaps="handled"
                contentContainerStyle={{
                ...
                renderItem={({item, index}) => this.renderCitiesData(item, index)}
              />
              {/* {cityData.map((eachlist, index) => {
                 this.renderCitiesData(eachlist,index);
              })} */}
            </View>
          </View>
        </Modal>
              ...
              <TouchableOpacity
                onPress={() =>
                  this.setState({
                    ...
                    selectedCitySearch: 0,
                  })
                }
                style={styles.textInputStyle}>
                <Text
                  style={{
                    color: pickUp ? 'black' : 'gray',
                    fontSize: Fonts.size.font14,
                  }}>
                  {pickUp ? pickUp : 'From City'}
                </Text>
                {/* <TextInput
                  style={{
                    color: pickUp ? 'black' : 'gray',
              ...
              <TouchableOpacity
                onPress={() =>
                  this.setState({
                    ...
                    searchText: '',
                    searchedCities: cityHistory,
                    selectedCitySearch: 1,
                  })
                }
                style={styles.textInputStyle}>
                <Text
                  style={{
                    color: drop ? 'black' : 'gray',
                    fontSize: Fonts.size.font14,
                  }}>
                  {drop ? drop : 'To City'}
                </Text>
        ...
        <ScrollView>
            ...
            <TouchableOpacity
              onPress={() => {
                if (originId && destId) {
                  ...
                  this.props.navigation.navigate('busAvailability', {
                    ...
                    <TouchableOpacity
                      onPress={() =>
                        this.props.navigation.navigate('busAvailability', {
                          ...
                          destId: item.destId,
                          date: item.date,
                          title: item.originN + ' To ' + item.destN,
                        })
                      }
                      style={{
                        marginHorizontal: 10,
          ...
          <View
            ...
            <Text style={{fontSize: 14, fontWeight: 'bold'}}>
              Discount :{' '}
              <Text style={{fontSize: 14, color: Colors.blueTheme}}> 5%</Text>{' '}
              <Text style={{fontSize: 14}}> (Max 50)</Text>{' '}
            </Text>
            <Text style={{fontSize: 14, color: Colors.blueTheme, marginTop: 7}}>
              Applicable to all bookings.
            </Text>
            <Text style={{fontSize: 13, color: 'gray', marginTop: 7}}>
              * Conditions Apply
            </Text>
          </View>
