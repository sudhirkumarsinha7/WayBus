        ...
        <View style={styles.container}>
          <View style={styles.headerView}>
            <View style={{flexDirection : 'row', alignItems:'center'}}>
              ...
              <Text style={styles.title}> Boarding & Dropping Point</Text>
            </View>  
          </View>
        </View>
            ...
            {selectedTab == 0 ? (
              <View style={{flex:1}}>
                ...
                <FlatList
                  ...
                  renderItem={({ item, index}) => (
                    <View   style={[styles.itemContainer, { borderWidth : selectedPickup == item.id ? 1.5 : 0 }]}>
              ...
              <View style={{flex:1}}>
                ...
                <FlatList
                  data={droppingPoints}
                  renderItem={({ item, index}) => (
                    <View   style={[styles.itemContainer, { borderWidth : selectedDrop == item.id ? 1.5 : 0 }]}>
                      <View style={{flex:1, flexDirection:'row',marginBottom:4, justifyContent : 'space-between'}}>
                        <Text style={[styles.grayContent, {flex:6}]}>{item.address}</Text>
                        <Text style={[styles.grayContent, {flex:1, textAlign:'right'}]}>{item.time}</Text>
                      </View>
