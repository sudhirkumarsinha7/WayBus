...
let genderRadio = [{ label: 'Male', value: 'M' }, { label: 'Female', value: 'F' }];
...
class addPassenger extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      selectedGender: { name: 'Male', value: 'M' },
      selectedIndex: null,
      showFare: false,
    };
  }
  ...
  render() {
    const { showFare, name, email, phone, passengerTypes, view, filterModalVisible, modalTitle, selectedGender, genderType } = this.state
    return (
      <View style={styles.mainContainer}>
        ...
        <View style={styles.locationContainer}>
          <View style={{ flex: 1 }}>
            ...
            {this.state.fareInfo ? (
              <View style={{ ...styles.itemContainer, marginTop: 0, paddingBottom: 10 }}>
                <TouchableOpacity style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }} onPress={() => this.setState({ showFare: !showFare })}>
                  <Text style={styles.grayContent}>Fare Details</Text>
                  <Image style={{ ...styles.smallIcon, margin: 0 }} source={showFare ? Images.upArrow : Images.downArrowIcon} />
                </TouchableOpacity>
                {showFare ? (
                        ...
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 10, marginBottom: 10 }}>
                          ...
                          <View style={{ flex: 1, height: 40, marginLeft:5 }}>
                            ...
                            <RadioForm
                              ...
                              buttonSize={12}
                              onPress={(value) => {
                                let val = this.state.genderType.find((gen) => gen.value == value);
                                this.addValues('sex', val, index);
                                this.setState({ selectedGender: val });
                              }}
                              formHorizontal={true}
                            />
