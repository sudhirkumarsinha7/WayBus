...
class seatSelection extends Component {
    ...
    getSchedule(scheduleId).then((res) => {
      ...
      if (res.payload.status == 200) {
        if (res.payload.data.result) {
          ...
          if(lowerB.length && lowerB[0].length>6){
            lowerB.shift();
          }
          console.log('lower B ' + JSON.stringify(lowerB));
          for (let i = 1; i < lowerB.length; i++) {
            console.log('lower I '+ i + ' '+ JSON.stringify(lowerB[i][0].split('|')[0]));
            
            
            if (lowerB[i - 1].length > lowerB[i].length) {
              let greeting = lowerB[i-1][0].split('|')[0];
              ...
              if(seatType === 'sleeper'){
                lowerB[i].unshift('|.GY-undefined@L');
              }else{
                lowerB[i].push('|.GY-undefined@L');
              }
  ...
  render() {
    ...
    const {date, title, depTime,travelsName} = this.props.route.params;
    console.log('lowerBerth ' + JSON.stringify(lowerBerth));
        ...
        <View style={styles.bodyContainer}>
          <View style={{flex: 1}}>
            ...
            <View
              ...
              <TouchableOpacity
              onPress ={()=>{this.setState({iscancelPolicy:!this.state.iscancelPolicy})}}
            ...
            {selectedSeats[0] ? (
             ...
             {this.state.iscancelPolicy ? (
              <View
                ...
                <Text>
                1. Less than  6 hours  from the station departure time:  100 % Cancellation Charges 
                </Text>
                <Text>
                2. Between 6 Hrs to 12 Hrs from the station departure time:  50 % Cancellation Charges 
                </Text>
                <Text>
                3. Between 12 Hrs to 24 Hrs from the station departure time:  30 % Cancellation Charges 
                </Text>
                <Text>
                4.Between 1 to 3 days before station departure time:  10 % Cancellation Charges 
                </Text>
                <Text>
               5.Between 3 to 7 days before station departure time:  10 % Cancellation Charges 
                </Text>
                <Text>
               6. Between 7 to 30 days before station departure time:  10 % Cancellation Charges 
                </Text>
              </View>
            ) : null}
          </View>
